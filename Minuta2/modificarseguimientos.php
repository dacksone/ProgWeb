...
<html lang="en">
  ...
  <body>
	...
	<br>
	<h1>Modificar seguimientos</h1> <br>
	
	<?php
    
	//$id_departamento = $_POST['id_departamento'];
	$id_seguimiento = $_GET['id_seguimiento'];
    $consulta1 = "SELECT * FROM seguimiento where id_seguimiento='$id_seguimiento'";
    //echo $consulta1;
    $columna1 = mysqli_query($conexion, $consulta1 ) or die ( "Error en consulta base de datos");
	// Bucle while que recorre cada registro y muestra cada campo en la tabla.
	echo "<form name='f1' id='f1' method='post' action ='modificarseguimientos2.php' >";
	while ($filas = mysqli_fetch_array( $columna1))
	{
    echo "<br>";
		echo " id seguimiento " ;
		echo "<input name ='id' type ='text' readonly value='".$filas["id_seguimiento"]."'>";
		echo "<br>";
		echo "<br>";
		echo " Nombre seguimiento " ;
		echo "<input name ='nombredeseguimiento' type ='text' value='".$filas["nombre"]."'>";
		echo "<br>";
		echo "Detalle   " ;		
		echo "<input name ='detalleseguimiento' type ='text' value='".$filas["detalle"]."'>";
		echo "<br>";
